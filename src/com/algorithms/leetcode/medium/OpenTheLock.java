...
public class OpenTheLock {
    ...
    public int openLock(String[] deadends, String target) {
        ...
        while (!queue.isEmpty()) {
            int size = queue.size();
            while (size > 0) {
                final String lockPosition = queue.poll();
                ...
                final StringBuilder builder = new StringBuilder(lockPosition);
                for (int i = 0; i < 4; i++) {
    ...
    private void process(int index, Queue<String> queue, Set<String> visited, Set<String> deadEnds, StringBuilder builder) {
        final char currentPosition = builder.charAt(index);
        final String clockwisePosition = builder.substring(0, index)
                + (currentPosition == '9' ? 0 : currentPosition - '0' + 1);
        visitAndAddToQueueIfPossible(clockwisePosition, queue, visited, deadEnds);

        final String counterClockwisePosition = builder.substring(0, index)
                + (currentPosition == '0' ? 9 : currentPosition - '0' - 1);
